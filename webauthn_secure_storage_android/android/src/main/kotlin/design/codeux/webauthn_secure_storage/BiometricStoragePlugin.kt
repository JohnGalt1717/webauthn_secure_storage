...
enum class AuthenticationError(vararg val code: Int) {
    ...
    companion object {
        fun forCode(code: Int) = values().firstOrNull { it.code.contains(code) } ?: Unknown
    }
}
...
class BiometricStoragePlugin : FlutterPlugin, ActivityAware, MethodCallHandler {
    ...
    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        ...
        val channel = MethodChannel(binding.binaryMessenger, "webauthn_secure_storage")
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        ...
        try {
            ...
            fun withStorage(cb: BiometricStorageFile.() -> Unit) {
                val name = getName()
                storageFiles[name]?.apply(cb)
                        ?: run {
                            logger.warn {
                                "User tried to access storage '$name', before initialization"
                            }
                ...
                worker(resultError) {
                            ...
                            if (options.androidAuthenticationValidityDuration != null) {
                                null
                                    ...
                                    try {
                                        ...
                                        logger.warn(e) {
                                            "Key was invalidated. removing previous storage and recreating."
                                        }
                                        deleteFile()
                    ...
                    if (cipher == null && !forceBiometricAuthentication) {
                        ...
                        try {
                            ...
                            logger.debug(e) {
                                "User requires (re)authentication. showing prompt ..."
                            }
                        }
                    }
            ...
            fun parseInitOptions() =
                    call.argument<Map<String, Any>>("options")?.let {
                        InitOptions(
                                androidAuthenticationValidityDuration =
                                        (it["androidAuthenticationValidityDurationSeconds"] as Int?)
                                                ?.seconds,
            ...
            when (call.method) {
                ...
                "init" -> {
                    ...
                    //                        .fromJsonValue(call.argument("options") ?:
                    // emptyMap<String, Any>())
                    //                        ?: InitOptions()
                ...
                "dispose" ->
                        storageFiles.remove(getName())?.apply {
                            dispose()
                            result.success(true)
                        }
                                ?: throw MethodCallException(
                                ...
                                withAuth(
                                        ...
                                        forceBiometricAuthentication =
                                                requiredArgument<Boolean>(
                                                        PARAM_FORCE_BIOMETRIC_AUTHENTICATION
                                                ),
                                ) {
                                    val ret =
                                            readFile(
                ...
                "exists" -> withStorage { result.success(exists()) }
                "delete" ->
                        withStorage {
                            if (exists()) {
    ...
    private fun canAuthenticate(initOptions: InitOptions): CanAuthenticateResponse {
        if (!initOptions.authenticationRequired) {
            ...
            logger.warn {
                "called `canAuthenticate` with initOptions.authenticationRequired == false. $initOptions"
            }
    ...
    private fun authenticate(
                ...
                BiometricPrompt(
                        ...
                        object : BiometricPrompt.AuthenticationCallback() {
                            ...
                            override fun onAuthenticationFailed() {
                                ...
                                // this just means the user was not recognised, but the O/S will
                                // handle feedback so we don't have to
                            }
                        }
                )
        ...
        if (cipher == null || options.androidAuthenticationValidityDuration != null) {
            ...
            logger.debug {
                "Authenticating without cipher. ${options.androidAuthenticationValidityDuration}"
            }
